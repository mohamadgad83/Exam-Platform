/**
 * ============================================
 * Exam Security Module - Anti-Cheating System
 * Exam Platform V2 - Preserved from V2
 * ============================================
 */
...
const ExamSecurity = {
    ...
    getViolationCount() { return this.state.violations; },
    ...
    _logViolation(type, message) {
        ...
        if (this.config.onViolation) this.config.onViolation(this.state.violations, message);
        if (this.state.violations >= this.config.maxViolations) this._triggerMaxViolations();
        this._sendToServer(entry);
    },

    _triggerMaxViolations() {
        this.disable();
        if (this.config.onMaxViolations) this.config.onMaxViolations(this.state.violations);
        if (typeof autoSubmit === 'function') autoSubmit('security_violation');
        if (typeof App !== 'undefined') {
            App.Toast.error('⚠️ تم اكتشاف محاولات غش متعددة! سيتم تسليم الامتحان تلقائياً.');
        }
    },
...
if (document.querySelector('.exam-container') || document.querySelector('#examTimer') || document.querySelector('[data-exam-mode]')) {
    ExamSecurity.init();
}
